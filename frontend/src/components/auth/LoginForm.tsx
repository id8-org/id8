...
import backgroundSvg from '@/assets/login-bg.svg';
...
export const LoginForm: React.FC<LoginFormProps> = ({ onSwitchToRegister }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div
        className="relative mx-auto w-full max-w-md p-10 rounded-2xl bg-white border border-slate-200 shadow-md flex flex-col items-center"
      >
        {/* Logo & branding */}
        <div className="flex flex-col items-center mb-8">
          <img src="/id8logo.png" alt="ID8 Logo" style={{ width: 96, height: 96, objectFit: 'contain' }} className="mb-2" />
          <span className="text-2xl font-bold text-blue-900 tracking-tight text-center" style={{ fontSize: '1.5rem', lineHeight: 1.2 }}>
            ID8 - Structured Intelligence
          </span>
        </div>
        {/* Headline and subheadline */}
        <h2 className="text-lg font-semibold text-slate-900 mb-1">Log in to your account</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-5">
          ...
          <Button
            type="submit"
            className="w-full py-3 mt-2 rounded-lg font-semibold text-white bg-blue-600 hover:bg-blue-700 shadow-none transition-all duration-200 focus:ring-2 focus:ring-blue-400"
            disabled={isLoading}
          >
        ...
        <div className="w-full text-center mt-6 text-sm text-slate-500">
          ...
          <button type="button" className="text-blue-600 font-semibold hover:underline ml-1" onClick={onSwitchToRegister}>
            Sign up
          </button>
        </div>
      </div>
